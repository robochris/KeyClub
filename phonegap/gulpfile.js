 ...
 */
gulp.task('build', ['optimize', 'images', 'fonts', 'phonegapFiles'], function() {
...
gulp.task('phonegapFiles', function() {
    gulp.src(config.client + 'config.xml')
    .pipe(gulp.dest(config.build));

    gulp.src(config.client + 'res/**/*')
    .pipe(gulp.dest(config.build + 'res/'));

    gulp.src(config.client + 'icon.png')
    .pipe(gulp.dest(config.build));
});
...
// gulp.task('optimize', ['inject', 'test'], function() {
    ...
    return gulp
        ...
        //.pipe($.uglify()) // todo: put this back after finding out what the error is
        .pipe(getHeader())
        .pipe(jsAppFilter.restore())

        // replace the return statement in settings-service.js
        .pipe($.replace(/\/\/ #BEGIN INJECT-ENVIRONMENT([\S\s]*)\/\/ #END INJECT-ENVIRONMENT/g, 'var environment = environments.' + getEnvironment() + ';' ))
...
function getEnvironment() {
    if (args.phonegapProd) {
        return 'phonegapProd';
    }
    if (args.phonegapDev) {
        return 'phonegapDev';
    }
    if (args.environmentLocal) {
        return 'local';
    }
    return 'web';
}
